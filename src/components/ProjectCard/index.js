    ...
    return (
      <S.ProjectCard horizontal={horizontal} markRed={markRed}>
        ...
        <Horizontal css={{ marginTop: 10 }} spaceAll={15}>
          {!isWebBased && <A.ActionIcon tip="Abrir en Archivos" icon={faFolder} onClick={project.openDir} />}
          {!isWebBased && <A.ActionIcon tip="Editar" icon={faCode} onClick={project.edit} />}

          {isWebBased && (
            <A.ActionIcon tip={`Abrir ${project.webUrl}`} icon={faGlobe} onClick={project.openWebUrl} />
          )}
