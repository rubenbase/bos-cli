...
export default types
  ...
  .actions(self => {
    return {
      ...
      killProcess: async () => {
        const { value: port } = await prompt('Kill el puerto', 'Port');
        if (port) {
          fkill(`:${port}`)
            .then(() =>
              Swal({
                title: `El puerto ${port} se liberó.`,
                type: 'success'
              })
            )
            .catch(() =>
              Swal({
                title: 'Error',
                text: `Couldn't kill el puerto ${port}.`,
                type: 'error'
              })
            );
        }
      ...
      importWorkspace: flow(function*({ type, groupName }) {
        ...
        toast({
          type: 'success',
          title: 'Espacio de trabajo añadido con éxito!'
        });
      }),
      ...
      createGroup: flow(function*() {
        const { value: name } = yield prompt('Título del grupo');
        if (name) {
          self.groups.push(
            Group.create({
              name
      ...
      removeProject: flow(function*(projectId) {
        const confirmed = yield confirmDelete('Atención', `Se eliminará de la app no del disco duro...`);
      ...
      renameProject: flow(function*(projectId, projectName) {
        ...
        const { value: name } = yield prompt('Renombrar', undefined, {
      ...
      renameGroup: flow(function*(groupName, groupId) {
        const { value: name } = yield prompt('Renombrar grupo', undefined, {
      ...
      deleteGroup: flow(function*(groupId) {
        const confirmed = yield confirmDelete(
          'Atención',
          `Borrará los proyectos de la app no del disco duro...`
        );
        if (confirmed) {
